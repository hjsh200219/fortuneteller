...
import type { SchoolCode } from '../../types/interpretation.js';
...
class DtsInterpreter extends BaseSchoolInterpreter {
  ...
  determineYongSin(pillars: SajuPillars, method: any): string {
  ...
  interpretHealth(_pillars: SajuPillars, yongSin: string, _settings: any): string {
    return `천간의 투출과 통근을 중시하여 ${yongSin} 기운의 건강법을 권장합니다.`;
  }

  interpretWealth(_pillars: SajuPillars, yongSin: string, _settings: any): string {
    return `천간의 흐름에 따라 ${yongSin} 방향의 재물 운이 형성됩니다.`;
  }

  interpretCareer(_pillars: SajuPillars, yongSin: string, _settings: any): string {
    return `천간 중심 해석으로 ${yongSin} 오행 관련 명예로운 직업이 적합합니다.`;
  }

  interpretRelationship(_pillars: SajuPillars, yongSin: string, _settings: any): string {
    return `천간의 조화를 중시하여 ${yongSin} 기운과 어울리는 관계가 좋습니다.`;
  }

  interpretFame(_pillars: SajuPillars, yongSin: string, _settings: any): string {
    return `천간의 투출이 명예와 직결됩니다. ${yongSin} 분야에서 사회적 인정을 받을 수 있습니다.`;
  }

  private createSimpleSajuData(pillars: SajuPillars): any {
    return {
      day: pillars.day,
      month: pillars.month,
